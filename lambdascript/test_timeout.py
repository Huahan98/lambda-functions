...
- place request tickets on the S3 queue with short TIMEOUT_LIMIT.
- run the lambda handler, the desired behavior:
    ...
    queue, the reason for scheduling failure (suffix of ticket name) is correctly set to "timeout".
...
CodeBuild jobs is 2. The Lambda Scheduler will only move tickets to the dead letter queue if timeout has been reached 
...
def test():
    ...
    # halt the program long enough to ensure that timeout limit has been reached for the request tickets
    time.sleep(2*TIMEOUT_LIMIT_IN_SECONDS)
    ...
    for dead_letter_key in dead_letter_keys:
        assert (
            re_pattern.match(dead_letter_key).group(1) == "timeout"
